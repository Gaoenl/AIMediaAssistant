...
重试语义(与方案一致):
- 瞬时异常(网络/依赖不可用)  → 返回 CONSUME_LATER,交给 RocketMQ 消费重试;
...
class GenerationListener(MessageListener):
    """生成任务监听器:按 taskId 拉参数、执行生成、回传结果。"""
...
def start_consumer() -> None:
    ...
    consumer.set_namesrv_addr(settings.rocketmq_nameserver)
    consumer.subscribe(settings.topic, GenerationListener())
    consumer.start()
